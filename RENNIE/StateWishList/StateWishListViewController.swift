...
class StateWishListViewController: UIViewController, UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    override func viewDidLoad() {
        ...
        self.lbl_stateName.text = self.stateName
    }
    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        if stateName == nil {
            print("state name is nil")
            return
        }
        
        self.clothing = self.realm.objects(WishData.self).filter("stateName == '\(self.stateName!)' AND categoryName == 'CLOTHING'")
        self.jewewlery = self.realm.objects(WishData.self).filter("stateName == '\(self.stateName!)' AND categoryName == 'JEWELERY'")
        self.armory = self.realm.objects(WishData.self).filter("stateName == '\(self.stateName!)' AND categoryName == 'ARMORY'")
        self.trinkets = self.realm.objects(WishData.self).filter("stateName == '\(self.stateName!)' AND categoryName == 'TRINKETS'")
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        if segue.identifier == "sw_festDetail" {
